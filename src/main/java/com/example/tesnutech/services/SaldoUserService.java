...
public class SaldoUserService {
    ...
    public ResponseEntity<Map<String, Object>> updateSaldo(String authorization, SaldoUserPojo param){
        Map<String, Object> response = new HashMap<>();
        ...
        try{
            ...
            if(statusAuth == HttpStatus.UNAUTHORIZED.value()){
                ...
                if (dataBalance == null) {
                    ...
                    param.setIdUser(dataBalance.getIdUser());
                    param.setTop_up_amount(dataBalance.getBalance() + param.getTop_up_amount());
                    saldoUserMapper.updateSaldoUser(param);

                    resData.put("balance", dataBalance.getBalance() + param.getTop_up_amount());

                    response.put("statusCode", HttpStatus.OK.value());
                    response.put("message", "topup saldo berhasil.");
